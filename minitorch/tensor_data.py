...
class TensorData:
    ...
    def shape_broadcast(shape_a: UserShape, shape_b: UserShape) -> UserShape:
        """Call the shape_broadcast"""
        return shape_broadcast(shape_a, shape_b)

    def index(self, index: Union[int, UserIndex]) -> int:
        """Get the single-dimensional index for the given multi-dimensional index.

        Args:
        ----
            index (Union[int, UserIndex]): Index or tuple of indices.

        Returns:
        -------
            int: Single-dimensional index.

        Raises:
        ------
            IndexingError: If index is out of bounds.
    ...
    def get(self, key: UserIndex) -> float:
        """Retrieve a value from the tensor at the given index."""
        x: float = self._storage[self.index(key)]
        return x

    def set(self, key: UserIndex, val: float) -> None:
        """Set a value to the storage of a Tensor."""
